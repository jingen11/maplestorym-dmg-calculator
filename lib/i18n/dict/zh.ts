...
const zh: Dictionary = {
  ...
  home: {
    metaTitle: "MapleStory M 伤害计算器 — BOSS 伤害、IED 与暴击",
    metaDescription:
      "输入属性，立刻看到对普通怪与 BOSS 的每次伤害：等级差修正、BOSS 防御（IED）、暴击最小–最大值与食物增益全部计入。免费，无需注册。",
    ...
    starforceLink: "星之力升星费用 →",
    starforceBlurb:
      "升到每一颗星通常要花多少金币——从当前星数到目标星数的期望与典型费用，附每次点击的降星与损坏概率。",
    faq: [
      {
  ...
  flames: {
    metaTitle: "MapleStory M 重生火焰概率与所需火焰数",
    metaDescription:
      "查看每次重生火焰的真实概率，以及大概要开多少次才出。MapleStory M 全部重生火焰选项，按装备部位与火焰等级分列，取自 Nexon 官方公示。",
    h1: "重生火焰概率",
    intro:
  ...
  cubes: {
    metaTitle: "MapleStory M 方块概率与所需方块数",
    metaDescription:
      "查看每颗方块的真实概率，以及大概要用多少颗才出。MapleStory M 全部潜能与附加潜能词条，按部位与等级分列，取自 Nexon 官方公示。",
    h1: "方块概率",
    intro:
  ...
  starforce: {
    metaTitle: "MapleStory M 星之力升星费用计算器 — 每星金币",
    metaDescription:
      "MapleStory M 升到每颗星要多少金币？从当前星数到目标星数的期望与典型星之力费用，附每次点击的降星与损坏概率。",
    h1: "星之力升星费用计算器",
    intro:
      "选择当前星数与目标星数，看看这段升星通常要花多少——期望金币、典型一局与倒霉一局，逐星列出——并附上每次点击的成功、降星与损坏概率。",
    ...
    breaks: "期望损坏次数",
    typicalHint:
      "典型值是 {runs} 局模拟的中位数；十局中有九局在“倒霉一局”的数字之内完成。",
    simCapped:
      "模拟在 ★{star} 停止——再往上，单局需要的点击次数太多，浏览器算不完。期望值仍覆盖每一颗星。",
    ...
    to: "目标星数",
    starValue: "★{star}",
    minigame: "我每次都能按中 +5% 小游戏",
    minigameHint:
      "——每次成功率乘以 1.05（30% 变为 31.5%）。多出的部分从“保持”概率中扣除，不影响降星与损坏概率。",
    repair: "每次损坏的修复费",
    repairHint:
      "装备损坏后、下一次点击前的修复金币。若有护盾符（Shielding Ward）让损坏无需花费，请填 0。",

    chartTitle: "升到每颗星所需金币",
    chartSubtitle: "从 ★{from} 起累计。悬停或点击某颗星查看数字。",
    scale: "刻度",
    scaleLog: "对数",
    scaleLinear: "线性",
    legendExpected: "期望",
    legendMedian: "典型（中位数）",
    legendBand: "10–90% 的局数",
    chartAria: "从 ★{from} 到 ★{to}，升到每颗星所需累计金币的图表",
    pointAria: "★{star}：期望 {expected}，典型 {median}，倒霉 {p90}",
    tooltipStep: "仅这一星：期望 {cost}，约 {taps} 次点击",
    tooltipNoSim: "未模拟",
    ...
    colP90: "倒霉累计",
    tableFootnote:
      "每行是从上一颗星出发的一次点击。累计从 ★{from} 起算，并包含每次损坏 {repair} 的修复费。",

    noteSource: "概率与费用来自{link}，最后核对于 {date}。",
    sourceLabel: "社区星之力费用表",
    dataNotes: [
      "每行是从上一颗星升到本星的一次强化点击：金币费用，以及成功、保持、降一星或损坏的概率。",
      "损坏时星数不变，但装备必须先花金币修复才能继续强化。",
      "来源将 ★11 与 ★12 列为无损坏概率，但这两行合计只有 95%，且其 +5% 表中两行均为 5% 损坏——此处采用 5%。",
      "期望值由概率精确解出。典型与倒霉数字来自你浏览器中按同一张表模拟的数千局升星。",
    ],

    steps: [
      {
        title: "设置当前与目标星数",
        body: "拖动两个滑块。顶部数字、图表和表格都会按这段升星重算——费用从你的当前星数起累计，而不是从零开始。",
      },
      {
        title: "说明你怎么点",
        body: "如果你能稳定按中 +5% 的时机小游戏，勾选该项；它把每次成功率乘以 1.05。把修复费填成一次损坏实际花你的金额——若有护盾符抵消，填 0。",
      },
      {
        title: "读懂三个数字",
        body: "期望是长期平均值，被少数倒霉局拉高。典型是中位数——一半玩家花得更少，一半更多。倒霉是十局中九局已完成的位置。按典型做预算，记住倒霉。",
      },
      {
        title: "读图",
        body: "每个点是首次到达该星的总花费。橙线是期望费用，蓝线是中位数局，阴影带是 80% 局数落入的范围。刻度默认为对数，因为每颗星都比上一颗贵好几倍——切到线性就能感受那道墙。",
      },
      {
        title: "查看每次点击的概率",
        body: "表格列出每颗星上单次点击的结果——成功、保持、降星、损坏——以及仅这一星在之前所有花费之上还预计要花多少。",
      },
    ],

    exampleIntro:
      "从 ★{from} 升到 ★{to}，不含小游戏加成，每次损坏修复费 {repair}：",
    exampleExpected: "期望费用",
    exampleTaps: "期望点击次数",
    exampleBreaks: "期望损坏次数",
    exampleLast: "仅最后一星",
    exampleShare: "占整段升星的 {share}%",
    exampleGap:
      "每一段星之力升星都是这个形状：过了 ★15，每颗星的花费大约等于之前所有星的总和，所以最后一星永远是账单的大头。",

    readExpected:
      "期望费用是长期平均——如果几千名玩家都走这段路，每人平均要花的钱。少数灾难局把它拉到远高于大多数人的实际花费。",
    readExpectedTerm: "期望费用",
    readTypical:
      "典型一局是模拟局的中位数——一半更便宜，一半更贵。倒霉一局是 90% 的局已完成的位置；剩下 10% 还要更久。",
    readTypicalTerm: "典型一局",
    readTypicalTerm2: "倒霉一局",
    readLog:
      "对数是默认刻度，因为每颗星都比上一颗贵好几倍——线性刻度下，最后一星之前的一切都会被压成一条平线。线性就是为了让你看到这一点。",
    readLogTerm: "对数",
    readLogTerm2: "线性",

    spendDrops:
      "从 ★10 起，点击失败可能让你掉一星，同一位置起也可能损坏装备。两者都已计入期望费用——降星意味着上一颗星要重新付一遍。",
    spendBreak:
      "损坏后星数保留，但下一次点击前必须付修复费。把修复费设为 0，就能看到护盾符对你究竟值多少。",
    spendTail:
      "按典型一局做预算，开始前先看清倒霉的数字。★17 及以上，一次长连降就能让整段花费翻倍——平均值不会警告你，阴影带会。",
    spendWall:
      "过了 ★20，每次点击成功率 1%、降星 40%。仅 ★21 一星的期望费用就超过从 ★0 到 ★20 整段升星的三十倍以上。",

    howToName: "如何估算 MapleStory M 星之力升星费用",
    howToDescription:
      "算出从当前星数升到目标星数通常要花多少金币，以及倒霉的尾部还会多花多少。",
    appName: "星之力升星费用计算器",

    faq: [
      {
        question: "MapleStory M 升到 17 星要多少金币？",
        answer:
          "从 ★0 起，升到 ★17 的期望费用约为 {to17}（含修复），典型一局约为 {to17typical}。★18 期望约 {to18}，★20 约 {to20}。在计算器里设置你自己的当前星数——从 ★10 起算的总额只比从 ★0 起低一点，因为低星相比之下几乎不要钱。",
      },
      {
        question: "MapleStory M 星之力的成功率是多少？",
        answer:
          "★1 为 100%，之后每星降 5%：★2 为 95%，★11 为 50%，★15 为 30%，★17 为 20%，★19 为 10%，★20 为 5%。从 ★21 起每次都是 1%。完整的每次点击表——成功、保持、降星、损坏——就在本页。",
      },
      {
        question: "星之力强化什么时候会降星或损坏？",
        answer:
          "从 ★10 起的点击（即冲 ★11 或更高）可能降一星或损坏装备。降星概率从 ★11 的 10% 升到 ★19–20 的 30%，★21 起为 40%；损坏概率 ★11 至 ★20 为 5%，★21 起 10%，★26 起 15%。损坏后星数保留，但下一次点击前必须修复。",
      },
      {
        question: "+5% 星之力小游戏值得做吗？",
        answer:
          "值得——它是成功率乘 1.05，所以 20% 变成 21%，多出的部分从保持概率中扣。这能把 ★0 到 ★20 的期望费用从约 {to20} 降到 {to20mg}，省下约 {mgSaving}%，代价只是按准一次时机。",
      },
      {
        question: "为什么期望费用比典型费用高那么多？",
        answer:
          "因为星之力的花费是偏斜的：大多数局平平无奇，但少数局在高星遇上长连降，花费是中位数的好几倍。这些局把平均值拉高。典型值是一半玩家能低于的数字；倒霉值是十分之九的玩家能低于的数字。",
      },
      {
        question: "21 星及以上要花多少？",
        answer:
          "从 ★21 起每次点击成功率 1%、降星 40%，所以仅 ★21 一星的期望费用约为 {star21}——是 ★0 到 ★20 整段的许多倍。计算器给出到 ★{max} 的期望值，但模拟的典型与倒霉数字会在单局需要数百万次点击的地方停止。",
      },
    ],
  },
};
