...
const zh: Dictionary = {
  ...
  flames: {
    ...
    steps: [
      ...
      {
        title: "在“任意一个”和“全部同时”之间做选择",
        body: "“任意一个”是默认值：只要你选中的词条出现一条即算命中。“全部同时”要求所选词条全部落在同一次火焰上，因此必须出现两个选项；再次点击某个数值即可要求它占满两个选项槽。两种读法都取自同一份官方概率表。",
      },
    ],
    ...
    readSlotsTerm2: "每次火焰",
    readModes:
      "任意一个 只要命中一条即算成功；全部同时 需要所选词条全部落在同一次火焰上，顶部数据也会切换为“全部命中·每次火焰”。数值上的 ×2 标记表示你要求它占满两个选项槽。",
  ...
  cubes: {
    ...
    cellAriaStack:
      "{option} {value}，概率 {prob}%，需要 {count} 条词条",
    stackBadge: "×{count}",
    ...
    steps: [
      ...
      {
        title: "在“任意一个”和“全部同时”之间做选择",
        body: "“任意一个”是默认值：只要你选中的词条出现一条即算命中。“全部同时”要求所选词条全部落在同一件装备上，再次点击某个数值即可要求它出现 2 或 3 条。两种读法都取自同一份官方概率表，区别只在于问题本身。",
      },
    ],
    ...
    readModes:
      "任意一个 只要命中一条即算成功；全部同时 需要所选词条全部落在同一件装备上，顶部数据也会切换为“全部命中·每颗方块”。数值上的 ×2 标记表示你要求它出现两条。",
