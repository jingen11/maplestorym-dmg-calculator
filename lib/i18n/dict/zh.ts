...
const zh: Dictionary = {
  ...
  flames: {
    ...
    metaDescription:
      "MapleStory M 每一个重生火焰选项及其精确概率，取自 Nexon 官方概率公示。选择装备部位与火焰等级，勾选你想要的选项，即可看到每次火焰的概率以及所需的火焰数量。",
    h1: "重生火焰概率",
    intro:
    ...
    spendAny:
      "选中多条表示 其中任意一条，而不是全部同时命中。在同一件装备上拿到两个指定选项要罕见得多，因为那需要一次双选项的结果。",
    spendAnyTerm: "其中任意一条",
    spendTier:
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          "选中多个数值表示“其中任意一个”，而不是“全部都要”。一个选项槽只会抽出一条，所以选中的概率会相加，每多勾一条目标就更容易。计算器目前还无法回答反向问题——同一件装备上同时拿到两个指定选项的概率——那要罕见得多，因为它需要一次双选项的结果。",
      },
    ],
  },
  ...
  cubes: {
    ...
    metaDescription:
      "MapleStory M 每一个潜能与附加潜能选项及其精确概率，取自 Nexon 官方概率公示。选择部位、等级与词条位置，勾选你想要的选项，即可看到每颗方块的概率以及所需的方块数量。",
    h1: "方块概率",
    intro:
